...
// characters and/or digits, the Playlist name is invalid and should not be added.
//
...
// When a selected Playlist is printed out in print_library (next function to implement),
...
//      - SUCCESS if a new Playlist node with the given name is successfully added.
//
...
// Library is selected (as you would have implemented in Stage 1 in add_playlist).
//
...
// Library is selected (as you would have implemented in Stage 1 in add_playlist).
//
// If the currently selected Playlist has no Playlist before it, change the selected
// Playlist to the last Playlist in the Library.
//
// For example, for a Library containing the following Playlists:
//      - playlist1 <--- this being the "selected" Playlist
//      - playlist2
//      - playlist3 
//
// Calling select_previous_playlist will make "playlist3" the new selected Playlist;
//      - playlist1 
//      - playlist2
...
// the position number only if the position number and the track length is valid.
//
...
// no Tracks in the Playlist, inserting into the 0th position will insert the Track.
// 
// For example:
// [ ] 0. playlist1
// [*] 1. playlist2
...
// [ ] 2. playlist3
//
// The position number is invalid if it is negative or greater than the number of
...
//      - ERROR_INVALID_INPUTS if the title/artist/position/track length is invalid.
//      - SUCCESS if a new Track node with the given information is successfully added.
// You should check for these cases in the same order as specified here.
//
...
// [ ] 2. playlist3
//
// The total length of the selected Playlist (playlist2) is 7 minutes and 25 seconds.
...
//    - m, n → 5
//    - r → 6
// 4. Replace all adjacent same digits with one digit, and then remove all the zero (0) digits
// 5. If the first digit matches the numerical encoding of the first letter,
//    remove the digit.
// 6. Append 3 zeros if result contains less than 3 digits. Remove all except first 
//    letter and 3 digits after it.
//
...
// You will also need to check that the given artist name contains only alphabetical 
// letters.
//
...
// The Tracks should be the same order as they were in the Playlist they came from.
//
// For example, a Library containing:
// [ ] 0. RoadTrip
...
// Soundex encoding of B421. BLACKPINK's (case insensitive) encoding is also B421.
// Hence the new Playlist BLOCKPINK contains both Tracks from BLOCKPINK and 
// BLACKPINK. 
//
// If there are no Tracks by the specified artist, a new Playlist with the artist 
// name should still be created without any Tracks and appended to the end of the 
// Library.
//
// The new Playlist should not be created and added if the artist name is invalid.
//
// The new Playlist should not be created and added if a Playlist with this artist
// name already exists.
//
// For this function, the search for artists should be case insensitive. 
...
// You can assume that all artists of Tracks already
// in the Library contain only alphabetical letters. You will also need to check that
// the given artist name contain only alphabetical letters.
//
// The function should return one of the following #defines from cspotify.h:
