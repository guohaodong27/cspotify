...
int main(void) {
    ...
    printf("| We are a local music streaming service that does not stream any music.  |\n");
    ...
    while (keepLooping) {
        ...
        if (command[0] == COMMAND_ADD_PLAYLIST 
            ...
            int numScanned = sscanf(command, "%c %s", &ignoreChar, newPlaylistName);
            if (numScanned != 2) {
            ...
            && (command[1] == ' ' || command[1] == '\n')) {
            printf("Your Library (Empty if nothing has been added/printed): \n");
        ...
        // STAGE 2
            ...
            int result = rename_playlist(library, oldPlaylistName, newPlaylistName);
            ...
            int result = add_track(library, title, artist, trackLength, position);
            if (result == ERROR_INVALID_INPUTS) {
...
void do_print_help(void) {
    ...
    printf(""
        "\n  %c <track>\n"
        "     Delete the first instance of the given track in the selected Playlist of the Library.\n",
    ...
    printf(""
        ...
        "     Cut the given track in selected Playlist and paste it into the given destination Playlist.\n",
